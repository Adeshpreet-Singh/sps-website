...
export default function Navbar() {
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 motion-reduce:transition-none animate-nav-enter ${
        scrolled
          ? "bg-white dark:bg-dark-surface shadow-[0_1px_8px_rgba(0,0,0,0.1)] dark:shadow-[0_1px_8px_rgba(0,0,0,0.4)] backdrop-blur-sm"
          : "bg-white/95 dark:bg-dark-surface/95 shadow-[0_1px_3px_rgba(0,0,0,0.08)] dark:shadow-[0_1px_3px_rgba(0,0,0,0.3)]"
      }`}
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        {/* Logo — pill badge */}
        <Link href="/" className="flex items-center gap-2 group" aria-label="SPS Smith Pro Services — Home">
      ...
      >
        ...
        <div className="mt-auto flex flex-col gap-3 border-t border-border dark:border-dark-border px-5 py-5">
          <a
            href={siteConfig.phoneLink}
            aria-label={`Call us at ${siteConfig.phone}`}
            className="inline-flex items-center gap-2 text-sm font-semibold text-navy dark:text-accent"
          >
            <Phone className="h-4 w-4" aria-hidden="true" />
            {siteConfig.phone}
          </a>
          <Link
            href="/contact"
            onClick={() => setMobileOpen(false)}
            className="rounded-full bg-accent px-5 py-2.5 text-center text-sm font-semibold text-white transition-all duration-200 hover:bg-accent-dark hover:shadow-md hover:-translate-y-0.5 active:translate-y-0 active:shadow-sm motion-reduce:transition-none motion-reduce:transform-none btn-press"
          >
            Get a Quote
          </Link>
        </div>
